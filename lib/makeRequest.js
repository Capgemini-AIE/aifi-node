...
function getRequestOpts(self, requestArgs, spec, overrideData) {
  ...
  const urlData = urlParams.reduce((urlData, param) => {
    ...
    if (typeof arg !== 'string') {
      throw new Error(
        `Aifi: Argument "${param}" must be a string, but got: ${arg} (on API request to \`${requestMethod} ${path}\`)`
      );
    }
  ...
  if (args.filter((x) => x != null).length) {
    throw new Error(
      `Aifi: Unknown arguments (${args}). Did you mean to pass an options object?`
    );
  }
